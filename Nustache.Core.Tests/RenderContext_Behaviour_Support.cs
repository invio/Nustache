...
namespace Nustache.Core.Tests
{
    ...
    public class RenderContext_Behaviour_Support
    {
        ...
        {
            var exception = Assert.Throws<NustacheDataContextMissException>(() =>
                Render.StringToString("before{{#.}}{{.}}{{/.}}after", null, new RenderContextBehaviour { RaiseExceptionOnDataContextMiss = true })
            );

            Assert.AreEqual(
                "Path : . is undefined, RaiseExceptionOnDataContextMiss : true.",
                exception.Message);
        }
        ...
        {
            var exception = Assert.Throws<NustacheEmptyStringException>(() =>
                Render.StringToString("before{{foo}}after", new { foo = string.Empty },
                    new RenderContextBehaviour { RaiseExceptionOnEmptyStringValue = true })
            );

            Assert.AreEqual(
                "Path : foo is an empty string, RaiseExceptionOnEmptyStringValue : true.",
                exception.Message
            );
        }
        ...
        public void It_throws_an_exception_when_there_is_no_data_and_the_render_context_behaviour_throw_on_miss_is_true()
        {
            ...
            Assert.AreEqual(
                "Path : foo is undefined, RaiseExceptionOnDataContextMiss : true.",
                exception.Message
            );
        }
        ...
        public void It_throws_an_exception_when_there_is_no_nested_data_and_the_render_context_behaviour_throw_on_miss_is_true()
        {
            ...
            Assert.AreEqual(
                "Path : foo.bar is undefined, RaiseExceptionOnDataContextMiss : true.",
                exception.Message
            );
        }
